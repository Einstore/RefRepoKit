...
public class RefRepo: RefRepoManager {
    
    public enum Error: WebError {
        ...
        public var code: String {
            switch self {
            case .rsaValidationFailed:
                return "ref_repo.rsa_validation_failed"
            case .onlyOneIdRsaKeySupported:
                return "ref_repo.only_one_rsa_key_supported"
            case .sshKeyHasNotBeenSpecified:
                return "ref_repo.no_ssh_key_specified"
            }
        }
    ...
    public convenience init(
        ...
        let shell = try Shell(conn, on: eventLoop)
        try self.init(
            shell,
            temp: temp,
            on: eventLoop,
    ...
    public func add(rsa arr: [(domain: String, sha: String?)]? = nil) -> EventLoopFuture<Void> {
        ...
        func run(_ output: String) -> EventLoopFuture<Void> {
            ...
            for rsa in arr {
                if !output.contains(rsa.domain) {
                    let future: EventLoopFuture<Void> = self.shell.run(bash: "ssh-keyscan \(rsa.domain) >> \(rsa.domain.safeText) ; ssh-keygen -lf \(rsa.domain.safeText)", output: nil).future.flatMap { output in
                        if let sha = rsa.sha, !output.contains(sha) {
                            return self.eventLoop.makeFailedFuture(Error.rsaValidationFailed(domain: rsa.domain))
                        }
                        return self.shell.run(bash: "cat \(rsa.domain.safeText) >> ~/.ssh/known_hosts", output: nil).future.void()
                    }
        ...
        return shell.run(bash: "cat ~/.ssh/known_hosts", output: nil).future.flatMap { output in
            return run(output)
    ...
    public func add(ssh key: String, workspace: String) -> EventLoopFuture<Void> {
        ...
        return shell.cmd.mkdir(path: folder, flags: "-p").flatMap { _ in
            ...
            return self.shell.upload(string: key.finished(with: "\n"), to: file).flatMap { _ in
                return self.shell.run(bash: "chmod 400 \(file.escapeSpaces)", output: nil).future.map { _ in
    ...
    func run(bash command: String) -> EventLoopFuture<Void> {
        return shell.run(bash: command, output: nil).future.map { out in
